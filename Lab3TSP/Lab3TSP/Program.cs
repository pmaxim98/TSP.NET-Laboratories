    ...
    class Program
    {
        ...
        static void TestManyToMany()
        {
            ...
            using (Model1Container context = new Model1Container())
            {
                ...
                Console.WriteLine("Association by IDs. Enter count:");

                int totalAssociations = int.Parse(Console.ReadLine());

                for (int i = 0; i < totalAssociations; ++i)
                {
                    Console.Write("Artist ID:");

                    Artist artist = artists[int.Parse(Console.ReadLine())];

                    Console.Write("Album ID:");

                    artist.Album = artist.Album ?? new List<Album>();

                    artist.Album.Add(albums[int.Parse(Console.ReadLine())]);
